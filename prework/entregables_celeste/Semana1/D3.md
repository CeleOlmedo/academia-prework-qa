...
1) Registro/Log in de usuario + impacto en la base de datos.
    ...
    o**Riesgo/impacto:** Alto. Es la "puerta" del sistema. Si falla, el usuario no 
puede acceder a sus datos, carrito, etc. por ende no efectuaría ninguna compra, lo 
que representa una pérdida.
    o**Frecuencia de uso:** Muy alta. Se utiliza cada vez que el usuario desea 
acceder al sistema.
...
2) Confirmación de compra + servicio de pago.
    ...
    o**Riesgo/impacto:** Crítico. Un error genera reclamos, pérdida de confianz
     y podría generar acciones legales por parte del comprador.
    o**Frecuencia de uso:** Alta. Es el paso final obligatorio para cada venta
     exitosa.
    o**Estabilidad del flujo:** Alta. Una vez definido el sistema de pagos, rara
     vez se cambia.


3) Cálculo de Total de carrito

o   **Riesgo/impacto:** Crítico. Cobrar de más o de menos genera problemas legales o 
contables.
o   **Frecuencia de uso:** Constante. Se ejecuta cada vez que alguien agrega un 
producto al carrito.
o   **Estabilidad del flujo:** Totalmente estable. La lógica detrás del cálculo de 
total no varía.
